...
YappMaster::YappMaster(vector<string> args, bool b_verb, bool b_test, int shmid, int shmsz):
...
void YappMaster::usage() {
  std::cerr << "Usage When Run Yapp in Master Mode:"
            << std::endl
            << std::endl
            << "yappd --mode=master"
            << std::endl
            ...
            << std::endl;
  std::cerr << "Usage When Run Yapp in Worker Mode:"
            << std::endl
            << std::endl
            << "yappd --mode=worker"
            << std::endl
...
YAPP_MSG_CODE YappMaster::parse_arguments()
{
...
#endif
    for (size_t c = 0; c < sizeof(YAPP_MASTER_OPTION_ENTRY) /
                           sizeof(char *); c++) {
      if (false == b_arg_valid) { break; }
      if (key_str != YAPP_MASTER_OPTION_ENTRY[c]) {
        continue;
      }
      switch ((YAPP_MASTER_OPTION_ENTRY_MAPPING)c) {
        case YAPP_MASTER_OPT_PORT: {
          if (true == StringUtil::is_integer(val_str)) {
            cfg_obj.port_num = atoi(val_str.c_str());
            break;
          }
        }
        case YAPP_MASTER_OPT_CONN_STR: {
          cfg_obj.zk_conn_str = val_str;
          break;
        }
        case YAPP_MASTER_OPT_THRD_POOL_SIZE: {
          if (true == StringUtil::is_integer(val_str)) {
            cfg_obj.thrd_pool_size = atoi(val_str.c_str());
            break;
          }
        }
        case YAPP_MASTER_OPT_ZKCFG: {
...
YAPP_MSG_CODE YappMaster::run() {
  ...
  if (true == b_verbose) {
    std::cout << "-- running yapp at master mode" << std::endl;
    print_arg_list();
  }
  ...
  YappServiceHandler * yapp_srv_hnd_ptr = new YappServiceHandler(
    cfg_obj, true, b_verbose, b_testing, shrd_mem_id, shrd_mem_sz
  );
  ...
  shared_ptr<YappServiceHandler> yapp_master_srv_hdler(yapp_srv_hnd_ptr);
  shared_ptr<TProcessor> yapp_master_processor(
    new YappServiceProcessor(yapp_master_srv_hdler)
  );
  /* 2-3, choose thread manager & set thead factory */
  shared_ptr<ThreadManager> yapp_master_thrd_manager =
      ThreadManager::newSimpleThreadManager(cfg_obj.thrd_pool_size);
  shared_ptr<PosixThreadFactory> yapp_master_thrd_factory =
      shared_ptr<PosixThreadFactory>(new PosixThreadFactory());
  yapp_master_thrd_manager->threadFactory(yapp_master_thrd_factory);
  yapp_master_thrd_manager->start();

  /* 2-4, initialize & start the server in non-blocking mode. */
  shared_ptr<TServerTransport> yapp_master_srv_trans(
      new TServerSocket(cfg_obj.port_num)
  );
  shared_ptr<TTransportFactory> yapp_master_trans_fac(
      new TBufferedTransportFactory()
  );
  TThreadPoolServer yapp_master_server(
    yapp_master_processor, yapp_master_srv_trans,   yapp_master_trans_fac,
    protocol_factory,      yapp_master_thrd_manager
  );

  if (true == b_verbose) {
    std::cout << "Starting Yapp Master Daemon on Port "
              << cfg_obj.port_num
              << " with the initial threading pool size "
              << cfg_obj.thrd_pool_size
              << std::endl;
  }
  yapp_master_server.serve();
  if (true == b_verbose) {
    std::cout << "Finish Running Yapp Master Daemon on Port "
              << cfg_obj.port_num
              << " with the initial threading pool size "
              << cfg_obj.thrd_pool_size
              << std::endl;
