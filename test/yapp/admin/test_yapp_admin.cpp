...
#include "test_yapp_admin.h"

using namespace yapp::admin;

CPPUNIT_TEST_SUITE_REGISTRATION(YappAdminTest);
 ...
 */
void YappAdminTest::setUp(void){
  set_test_env();
  load_test_cases();
}

void YappAdminTest::load_test_cases(void) {
  /**
  ...
  test_case_cnt = sizeof(YAPP_ADMIN_PARAM_ARR_SET) / sizeof(char **);
  yapp_admin_params_list_arr_ptr = new vector<string>[test_case_cnt];
  for (int i = 0; i < test_case_cnt; i++) {
    for (int c = 0; c < YAPP_ADMIN_PARAM_ARR_SIZE_SET[i]; c++) {
      yapp_admin_params_list_arr_ptr[i].push_back(
        YAPP_ADMIN_PARAM_ARR_SET[i][c]
      );
    }
    yapp_admin_arr.push_back(
      YappAdmin(yapp_admin_params_list_arr_ptr[i], true, true)
    );
  }

  /**
   * 2. prepare the yapp master/worker server object for those upcoming tests.
   */
...
void YappAdminTest::set_test_env(void) {
...
void YappAdminTest::tearDown(void){
  delete[] yapp_params_list_arr_ptr;
  delete[] yapp_admin_params_list_arr_ptr;
}

void YappAdminTest::test_parse_arguments(void){
  for (size_t i = 0; i < yapp_admin_arr.size(); i++) {
#ifdef DEBUG_YAPP_CLIENT
    std::cerr << "Arguments List ";
    for (int c = 0; c < YAPP_ADMIN_PARAM_ARR_SIZE_SET[i]; c++) {
      std::cerr << yapp_admin_params_list_arr_ptr[i][c] << " ";
    }
    std::cerr << std::endl;
#endif
    CPPUNIT_ASSERT_EQUAL(YAPP_MSG_SUCCESS, yapp_admin_arr[i].parse_arguments());
  }
...
void YappAdminTest::test_run(void) {
  ...
  /** 3. load all the logged job/task/proc. handle used for admin. testing. **/
  load_all_proc_hndl_for_admin_test();
...
bool YappAdminTest::check_progress(ZkClusterProxy * zkc_proxy_ptr) {
  bool status = true;
...
#ifdef DEBUG_YAPP_CLIENT
  std::cerr << ">>>> ENTER YappAdminTest::check_progress" << std::endl;
#endif
...
void YappAdminTest::sanity_check(void)
{
  ConfigureUtil cfg_util;
  string cfg_file(INPUTS_FILE_ENTRY[TEST_LOAD_ZK_CLUSTER_CFG_FILE_IDX]);
  CPPUNIT_ASSERT(true == cfg_util.load_zk_cluster_cfg(cfg_file));
...
void YappAdminTest::load_all_proc_hndl_for_admin_test(void) {
  ConfigureUtil cfg_util;
  string cfg_file(INPUTS_FILE_ENTRY[TEST_LOAD_ZK_CLUSTER_CFG_FILE_IDX]);
  CPPUNIT_ASSERT(true == cfg_util.load_zk_cluster_cfg(cfg_file));
  ZkClusterProxy * zk_ptr = new ZkClusterProxy(true);
  ...
  CPPUNIT_ASSERT_EQUAL(YAPP_MSG_SUCCESS,
    zk_ptr->sync(zk_ptr->get_rfile_task_queue_path_prefix())
  );
  CPPUNIT_ASSERT_EQUAL(YAPP_MSG_SUCCESS,
    zk_ptr->get_node_arr(
      zk_ptr->get_rfile_task_queue_path_prefix(), task_hndls_arr
    )
  );
  CPPUNIT_ASSERT_EQUAL(YAPP_MSG_SUCCESS,
    zk_ptr->get_node_arr(
      zk_ptr->get_job_path_prefix(), jobs_hndls_arr
    )
  );
  CPPUNIT_ASSERT_EQUAL(jobs_hndls_arr.size(), task_hndls_arr.size());


  vector<string> proc_node_arr;
  string tskhd_in_job, max_line_idx, tskhd_in_run,
         cur_line_idx, nxt_line_idx, tot_proc_cnt;

  for (size_t i = 0; i < jobs_hndls_arr.size(); i++) {
    CPPUNIT_ASSERT_EQUAL(0, (int)task_hndls_arr[i].find(jobs_hndls_arr[i]));
    CPPUNIT_ASSERT_EQUAL(YAPP_MSG_SUCCESS,
      zk_ptr->get_node_arr(
        zk_ptr->get_rfile_task_queue_path_prefix() + "/" +
          task_hndls_arr[i] + "/proc_arr",
        proc_node_arr
      )
    );
    for (size_t c = 0; c < proc_node_arr.size(); c++) {
      RangeFileTaskDataParser::parse_rfile_task_data_str(
        proc_node_arr[c], tskhd_in_job, max_line_idx, tskhd_in_run,
                          cur_line_idx, nxt_line_idx, tot_proc_cnt
      );
      proc_hndls_arr.push_back(tskhd_in_job);
    }
    proc_node_arr.clear();
  }
  /**
   * every vector pointer from yapp_admin_params_list_arr_ptr represents a uniq
   * branch in admin, eg. qstat, print, pause, resume, kill, restart, purge.
   * for every functional branch, we need to test it with diff. kinds of input,
   * either job hndl, task hndl or proc hndl, or any of the combination.
   */
  size_t test_case_cnt = sizeof(YAPP_ADMIN_PARAM_ARR_SET) / sizeof(char **);
  for (size_t i = 0; i < test_case_cnt; i++) {
    size_t cur_proc_hndl_idx = 0, cur_task_hndl_idx = 0, cur_job_hndl_idx = 0;
    for (; cur_job_hndl_idx < jobs_hndls_arr.size(); cur_job_hndl_idx++) {
      for (; cur_task_hndl_idx < task_hndls_arr.size(); cur_task_hndl_idx++) {
        if (0 != task_hndls_arr[cur_task_hndl_idx].find(
                 jobs_hndls_arr[cur_job_hndl_idx])) { break; }
        for (; cur_proc_hndl_idx < proc_hndls_arr.size(); cur_proc_hndl_idx++) {
          if (0 != proc_hndls_arr[cur_proc_hndl_idx].find(
                   task_hndls_arr[cur_task_hndl_idx])) { break; }
          yapp_admin_actual_cases.push_back(
            vector<string>(yapp_admin_params_list_arr_ptr[i].begin(),
                           yapp_admin_params_list_arr_ptr[i].end())
          );
          yapp_admin_actual_cases.back().push_back(proc_hndls_arr[cur_proc_hndl_idx]);
        }
        yapp_admin_actual_cases.push_back(
          vector<string>(yapp_admin_params_list_arr_ptr[i].begin(),
                         yapp_admin_params_list_arr_ptr[i].end())
        );
        yapp_admin_actual_cases.back().push_back(task_hndls_arr[cur_task_hndl_idx]);
      }
      yapp_admin_actual_cases.push_back(
        vector<string>(yapp_admin_params_list_arr_ptr[i].begin(),
                       yapp_admin_params_list_arr_ptr[i].end())
      );
      yapp_admin_actual_cases.back().push_back(jobs_hndls_arr[cur_job_hndl_idx]);
    }
  }

#ifdef DEBUG_YAPP_CLIENT
  int cnt = 0;
  std::cerr << ">>>> TESTING CASES LOADED: " << std::endl;
  for (size_t i = 0; i < yapp_admin_actual_cases.size(); i++) {
    cnt += 1;
    std::cerr << "<<<< no. " << cnt << ": ";
    for (size_t c = 0; c < yapp_admin_actual_cases[i].size(); c++) {
      std::cerr << yapp_admin_actual_cases[i][c] << " ";
    }
    std::cerr << std::endl;
  }
#endif
...
void YappAdminTest::print_queue_stat(ZkClusterProxy * zk_ptr) {
...
void YappAdminTest::start_yapp_service(void) {
#ifdef DEBUG_YAPP_CLIENT
  std::cerr << std::endl;
#endif
  for (int i = 0; i < NUM_OF_NODES; i++) {
...
void * YappAdminTest::thread_yapp_service(void * yapp_worker_ptr) {
...
void YappAdminTest::stop_yapp_service(void) {
